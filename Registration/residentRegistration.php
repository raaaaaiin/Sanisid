...
include('../Resident/connections.php');

function decrypts($hexString) {
    $string = '';
...
if(isset($_GET['ref'])){
    ...
    $stats = $_GET['ref'];
    $rowse = mysqli_query($db, "SELECT *  FROM `request_acc` where id = '".$stats."' and status=1;" );
    
}else{
    $rowse = mysqli_query($db, "SELECT *  FROM `request_acc` where id = '".decrypt()."';" );
}

if ($rowse === false) {
} else if (mysqli_num_rows($rowse) == 0) {
    // no rows returned, execute redirect
    redirect();
} else {
$rowe = mysqli_fetch_array( $rowse );
}
$fname = isset($rowe['first_Name']) ? $rowe['first_Name'] : redirect();
$lname = isset($rowe['last_Name']) ? $rowe['last_Name'] : redirect();
$mname = isset($rowe['middle_Name']) ? $rowe['middle_Name'] : redirect();
$email = isset($rowe['email_Add']) ? $rowe['email_Add'] : redirect();
$proof = isset($rowe['proof']) ? $rowe['proof'] : redirect();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $suffix = mysqli_fetch_array(mysqli_query($db, "SELECT suffix_ID FROM `ref_suffixname` WHERE suffix = '$isuffix'"), MYSQLI_ASSOC)['suffix_ID'] ?? "";
    $res_suffix = $suffix;
    
    $igender = $_POST["res_gender"] ?? "";
    $gender = mysqli_fetch_array(mysqli_query($db, "SELECT gender_ID FROM `ref_gender` WHERE gender_Name = '$igender'"), MYSQLI_ASSOC)['gender_ID'] ?? "";
    $res_gender = $gender;
    
    $res_bdate = $_POST["res_bdate"] ?? "";
    
    $icstatus = $_POST["res_civilstatus"] ?? "";
    $cstatus = mysqli_fetch_array(mysqli_query($db, "SELECT marital_ID FROM `ref_marital_status` WHERE marital_Name = '$icstatus'"), MYSQLI_ASSOC)['marital_ID'] ?? "";
    $res_civilstatus = $cstatus;
    
    $res_contactnum = $_POST["res_contactnum"] ?? "";
    
    $ictype = $_POST["res_contacttype"] ?? "";
    $ctype = mysqli_fetch_array(mysqli_query($db, "SELECT contactType_ID FROM `ref_contact` WHERE contactType_Name = '$ictype'"), MYSQLI_ASSOC)['contactType_ID'] ?? "";
    $res_contacttype = $ctype;
    
    $irel = $_POST["res_religion"] ?? "";
    $relig = mysqli_fetch_array(mysqli_query($db, "SELECT religion_ID FROM `ref_religion` WHERE religion_name = '$irel'"), MYSQLI_ASSOC)['religion_ID'] ?? "";
    $res_religion = $relig;
    
    $ioccst = $_POST["res_occupationstatus"] ?? "";
    $occst = mysqli_fetch_array(mysqli_query($db, "SELECT occuStat_ID FROM `ref_occupation_status` WHERE occuStat_Name = '$ioccst'"), MYSQLI_ASSOC)['occuStat_ID'] ?? "";
    $res_occupationstatus = $occst;
    
    $iocc = $_POST["res_occupation"] ?? "";
    $occ = mysqli_fetch_array(mysqli_query($db, "SELECT occupation_ID FROM `ref_occupation` WHERE occupation_Name = '$iocc'"), MYSQLI_ASSOC)['occupation_ID'] ?? "";
    $res_occupation = $occ;
    ...
    $citi = mysqli_fetch_array(mysqli_query($db, "SELECT country_ID FROM `ref_country` WHERE country_citizenship = '$iciti'"), MYSQLI_ASSOC)['country_ID'] ?? "";
...
If($res_fname  && $res_lname  && $res_gender && $res_bdate && $res_civilstatus ){
  
    
    $ETOna = $proof;
     $id = $_GET['ref'];
        $query=mysqli_query($db,"INSERT INTO resident_detail(res_ID,res_Img, res_fName, res_mName,res_lName,suffix_ID, gender_ID, res_Bday, marital_ID,religion_ID,res_Height,res_Weight, occuStat_ID,occupation_ID,country_ID,Status,res_Age) VALUES('$id','$ETOna','$res_fname','$res_mname','$res_lname','$res_suffix','$res_gender','$res_bdate','$res_civilstatus','$res_religion','$res_height', '$res_weight','$res_occupationstatus','$res_occupation','$res_citizenship','Active','$res_age') ");
        $query=mysqli_query($db,"INSERT INTO resident_contact(contact_ID,contact_telnum,res_ID,contactType_ID,country_ID) VALUES('$cid','$res_contactnum','$id','$res_contacttype','$res_citizenship') ");
        $query=mysqli_query($db,"INSERT INTO resident_address(address_ID,address_Unit_Room_Floor_num,res_ID,address_BuildingName,address_Lot_No,address_Block_No,address_Phase_No,address_House_No,address_Street_Name,address_Subdivision,country_ID,purok_ID,region_ID,addressType_ID) VALUES('$aid','$res_unit','$id','$res_building',' $res_lot',' $res_block','$res_phase','$res_houseno','$res_street','$res_subd','$res_citizenship','$res_purokno','$res_region','$res_address') ");
...
function convertfile(){
    $loc = $GLOBALS['proof'];
    $img = resize_image("../asset/requestImg/$loc", 300, 300);
    ob_start();
    imagejpeg($img);
    $image = ob_get_clean();
    ob_end_clean();
...
<html lang="en">
  ...
  <form id="hoy" method="POST" enctype="multipart/form-data" action="">
      <div id="app">
...
<center>
                                  <div id="Changepic" style="background-color: #e7effc;width:300px;height:300px;" class="formdes-mb-5 formdes-file-input h-100">
                                      ...
                                      <label id="RemovePic" style="padding:0" for="file" class="h-100" id="bgClick" onclick="imgclick()">
                                          <img style='width:100%;height:100%' src='../asset/requestImg/<?= $proof?>'>
                                      </label>
                                  </div></center>
                                          ...
                                          <div class="form__form-row col-1">
                                              ...
                                              <div class="form__form-row">
                                                  <div class="form-element col-2" name="position-firstName">
                                                      ...
                                                      <input type="text" tabindex="" class="data-hj-whitelist" autocomplete="off" name="res_fname" value="<?= $fname?>" placeholder="First Name" required>
                                                  </div>
                                                  <div class="form-element col-2" name="position-firstName">
                                                      <label class="">Middle name</label>
                                                      <input type="text" tabindex="" class="data-hj-whitelist" autocomplete="off" name="res_mname" value="<?= $mname?>"  placeholder="Middle Name">
                                                  </div>
                                                  <div class="form-element col-1" name="position-lastName">
                                                      <label class="">Last name *</label>
                                                      <input type="text" tabindex="" class="data-hj-whitelist" autocomplete="off" name="res_lname" value="<?= $lname?>" placeholder="Last Name" required>
                                                  </div>
                                              </div>
                                          </div>
                                          <div class="form__form-row col-1">
                                                      ...
                                                      <div class="form__form-row form-element--desktop col-1" style="margin-bottom: 0px;">
                                                          <div class="col-1">
                                                              <div class="col-2 form-element form-element--desktop">
                                                                  <div  style="width:33%"  class="form-element date--day" name="position-meta.dateOfBirthDay">
                                                                      <label class="">
                                                                          <div  >Suffix</div>
                                                                      </label>
                                                                  ...
                                                                  <div  style="width:33%"   class="form-element date--month" name="position-meta.dateOfBirthMonth">
                                                                      <label class="">
                                                                          <div  >Age</div>
                                                                      </label><br>
                                                                      <input type="text" style="width:100%;" class="data-hj-whitelist" name="res_age" value="0" id="res_age">


                                                                  </div>
                                                                  <div  style="width:33%"  class="form-element date--year" name="position-meta.dateOfBirthYear">
                                  ...
                                  <button id="subsub" type="submit" class="btn__large button--purple">
...
const subsubButton = document.getElementById("subsub");

form.addEventListener("submit", function(event) {
  if (subsubButton.form === this) {
    event.preventDefault();
    alert("Wait for administrator to approve your registration");
    document.getElementById("hoy").submit();
  }
});
    function imgclick(){
