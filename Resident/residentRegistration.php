...
if(isset($_GET['id'])){
    $stats = $_GET['id'];
    $rowse = mysqli_query($db, "SELECT *  FROM `request_acc` where user_name = '$stats';" );
}else{
    $rowse = mysqli_query($db, "SELECT *  FROM `request_acc` where id = '".decrypt()."';" );
    echo '<script>alert("Your account is now activated please log in");</script>';
    header('Location:../login.php');
    
}
$rowe = mysqli_fetch_array( $rowse );
$fname = $rowe['first_Name'];
$lname = $rowe['last_Name'];
$mname = $rowe['middle_Name'];
$email = $rowe['email_Add'];
$proof = $rowe['proof'];
...
If($rid && $res_fname  && $res_lname  && $res_gender && $res_bdate && $res_civilstatus  && $res_religion  && $res_citizenship){
    ...
    $ETOna = addslashes(convertfile());
     @$id = $_SESSION['id'] ? $_SESSION['id'] : decrypt(); 
     @$user = $_SESSION['USER'] ? $_SESSION['USER'] : decrypt(); 
        $query=mysqli_query($db,"INSERT INTO resident_detail(res_ID,res_Img, res_fName, res_mName,res_lName,suffix_ID, gender_ID, res_Bday, marital_ID,religion_ID,res_Height,res_Weight, occuStat_ID,occupation_ID,country_ID,Status,res_Age) VALUES('$rid','$ETOna','$res_fname','$res_mname','$res_lname','$res_suffix','$res_gender','$res_bdate','$res_civilstatus','$res_religion','$res_height', '$res_weight','$res_occupationstatus','$res_occupation','$res_citizenship','Active','$res_age') ");
        $query2=mysqli_query($db,"update accounts set status = 1 where username='$user'");
        if($query2){
