...
<div id="over" class="woberlay show"></div>
<div id="spanner" class="hisspanner show">
    <div class="loader"></div>
    <p style="color:white;font-size:1em">Email is sending, please wait.</p>
</div>
...
<div id="simpleModal" class="mdl-overlay">
  ...
  height: 250px;
            ...
            *Note If you want to resched an appointment just select a new date then click approve
            *If you have no clarification just click approve
            <br>

            <form method="POST" target="_blank" action="appointment.php">

                Time:    <input id="thime" name="time" type="text" value=""/><br>
                Date:    <input id="date" name="date" type="date" value=""/><br>
                <input hidden id="name" name="name" value=""/>
                <input hidden id="email"  name="email" value=""/>
                <input hidden id="purpose"  name="purpose" value=""/>
                Remarks: <input id="requirement"  name="requirement" value=""/><br>
                <input hidden id="verdict"  name="verdict" value=""/>
...
<div clas="wrpr">
    <div class = "cnt-fld p-4">
        ...
        <div class="r">
            <div class="c-12 ">
                <span ><h1 class="ml-4">Pending Appointments</h1></span>
                    ...
                    if($resultCheck_ret > 0){
                        while($row_ret = mysqli_fetch_assoc($result_ret)){
                            ?>  <li class="tbl-rw b psbr"style="    align-items: center;">
                                ...
                                <div class="c c-2"><span><?php echo $row_ret['purp']?></span></div>
                                ...
                                <div class="c c-1">
                                    ...
                                    <input type="text" style="display: none;"  name="resitime" id="<?php echo $row_ret['id'];?>resitime"/>
                                    <input type="text" style="display: none;"  name="residate" id="<?php echo $row_ret['id'];?>residate"/>
                                    <input type="text" style="display: none;"  name="resiquirement" id="<?php echo $row_ret['id'];?>resiquirement"/>
                                    <input type="text" style="display: none;" value="<?php echo $row_ret['purpose']?>" name="purpose"/>
                                    <input type="text" onclick="Modalshow('<?php echo $time;?>','<?php echo $app_date;?>','<?php echo $name;?>','<?php echo $email;?>','<?php echo $purpose;?>','requirements','Approved','<?php echo $id;?>')" class="button app w-100" value="Update" />
                                    <input  id="app<?php echo $row_ret['id'];?>" type="submit" name="approved"
                                            class="button app" value="✖" hidden />
                                    <input  id="rej<?php echo $row_ret['id'];?>" type="submit" name="reject"
              ...
              "src="data:image/jpg;charset=utf8;base64,<?php echo base64_encode($row_ret['proof']); ?>" /> <br>-->
                ...
                function reject() {
                    ...
                    if ($db->query($sql) === TRUE)
                    {
                        ...
                        echo "<script>alert('You successfully rejected an appointment');</script>";
                        sendMail($email,$name,$date,$time,$purpose,$requirements,'Rejected',$value);
...
<script>
    ...
    function Modalshow(a,b,c,c1,d,e,f,g){
        var date = new Date(b+"T"+a);
        let dateInputFormatted = date.toISOString().split('T')[0];


        document.getElementById('thime').value= a;
        document.getElementById('date').value= dateInputFormatted;
    ...
    function formsubmitter(){
        ...
        alert('Please wait for the confirmation that email is sent');
        document.getElementById(selected+'resitime').value = document.getElementById('thime').value;
